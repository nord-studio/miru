...
use crate::cron::manager::AsyncJob;
use crate::cron::manager::Interval;
use crate::cron::manager::{
    async_job::JobFuture,
    ...
    Tp: TimeProvider,
{
    ...
    pub fn get_jobs(&self) -> &[AsyncJob<Tz, Tp>] {
        &self.jobs
    }

    /// Get a specific job by index
    pub fn get_job(&self, index: usize) -> Option<&AsyncJob<Tz, Tp>> {
        self.jobs.get(index)
    }

    /// Remove a job from the scheduler by index
    pub fn remove_job(&mut self, index: usize) -> Option<AsyncJob<Tz, Tp>> {
        Some(self.jobs.remove(index))
    }

    /// Update a job in the scheduler by index
    pub fn update_job(&mut self, index: usize, job: AsyncJob<Tz, Tp>) -> Option<AsyncJob<Tz, Tp>> {
        self.jobs.get_mut(index).map(|j| std::mem::replace(j, job))
    }
