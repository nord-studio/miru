...
use crate::{
    cron::manager::job::Job,
    cron::manager::job_schedule::{JobSchedule, WithSchedule},
    cron::manager::timeprovider::{ChronoTimeProvider, TimeProvider},
    cron::manager::Interval,
};
    ...
    Tp: TimeProvider,
{
    pub(crate) fn new(ival: Interval, tz: Tz) -> Self {
        AsyncJob {
            jid: generate_id().parse().expect("Failed to parse ID"),
